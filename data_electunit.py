...
def need_calculates_one_ele(pref_1,asitis_df,ele_num):
    ...
    #当選者の基本変数に欠損があるかどうか
    nan_bools_win = target_df[target["win"] == True][needed_variables_l].isnull().any()
    ...
    #選挙ごとの競争率
    compe_rate_data = n_cand_data/n_wins
    ...
    #ここまでの変数をリスト化
    calculates_l = [ele_ymd,n_districts,n_cand_data,n_wins_data,compe_rate_data,no_voting_ratio_win,no_voting_ratio_districs,minasi_nv_ratio_districts,ratio_women_cand,ratio_women_wins,ratio_inc_cand,,ratio_new_cand,ratio_form_cand,ratio_inc_wins,ratio_new_wins,ratio_form_wins,ratio_inc_votes,ratio_new_votes,ratio_form_votes,ratio_inc_votes_percand,ratio_new_votes_percand,ratio_form_votes_percand,all_votes,n_circles] + age_stats_cand + age_stats_wins
...
def circle_info_all_eles(pref_1,asitis_df):
    ...
    #concate
    result_df = pd.concat([rowele_df,voteshare_df,voteshare_ave_df,n_cand_df,n_wins_df],axis=1,keys=["得票数","得票率","一人あたり得票率","立候補者数","当選者数"])
